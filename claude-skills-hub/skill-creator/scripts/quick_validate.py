...
def validate_skill(skill_path):
    ...
    if not content.startswith('---'):
        return False, "未找到 YAML 前置内容"

    # Extract frontmatter
    match = re.match(r'^---\n(.*?)\n---', content, re.DOTALL)
    if not match:
        return False, "前置内容格式无效"
    ...
    try:
        ...
        if not isinstance(frontmatter, dict):
            return False, "前置内容必须是 YAML 字典"
    except yaml.YAMLError as e:
        return False, f"前置内容中的 YAML 无效: {e}"
    ...
    if unexpected_keys:
        return False, (
            f"SKILL.md 前置内容中的意外键: {', '.join(sorted(unexpected_keys))}。"
            f"允许的属性为: {', '.join(sorted(ALLOWED_PROPERTIES))}"
        )

    # Check required fields
    if 'name' not in frontmatter:
        return False, "前置内容中缺少 'name'"
    if 'description' not in frontmatter:
        return False, "前置内容中缺少 'description'"
